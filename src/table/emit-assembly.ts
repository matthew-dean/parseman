...
break}`
        ...
        const fallbackCase = other === undefined ? '' : `default:${otherRouted
          ? routedCall(other)
          : plainCall(other)}`
        const tokenBacked = tokenChoiceDispatches.has(ip)
        const directToken = tokenBacked && other === undefined
          && spec.routed.length === n && spec.routed.every(r => r === 1)
        const armSelection = tokenBacked
...
${fold}${chain}}`
          : `let arm=${bk}.get(key)
${fold}${chain}`
        const selectorAndArm = directToken
          ? (() => {
              const selectorIp = code[ip + 1]!
              const lineFlags = code[selectorIp + 4]!
              const hasSuffix = (lineFlags & 4) !== 0
              const suffixExpected = hasSuffix ? fxRef(code[selectorIp + 3]!) : ''
              return `let key,arm,selEnd
const tp=_pfTokDispatch===${ip}&&_pfTokInput===input&&_pfTokPos===pos
if(tp){
const tr=_pfTokPacked
const sm=${hasSuffix ? 'tr%2===1' : 'false'}
selEnd=(tr-(sm?1:0))/2
key=_pfTokValue
arm=_pfTokArm
...
const key=sv
${armSelection}`
        // The selector runs once unless the enclosing choice has already run
        // its exact lexical body and classified an all-routed/no-fallback arm.
        // That bounded handoff enters the SAME arm switch and rollback protocol
        // below; no per-choice or per-transform copy of the dispatch shell is
        // emitted. A routed arm rewinds the selector capture (empty on the
        // handoff path) and owns the token through `OP_ROUTED`.
        return `${head}

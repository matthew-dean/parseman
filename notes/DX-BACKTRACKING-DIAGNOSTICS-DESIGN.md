...
- **The snapshot allowlist is the SINGLE per-choice suppression mechanism for
  gating findings.** `analyzeGating(entry, { accept })` / `compile(g, { gating: {
  level, accept } })` take a set of choice `id`s that are intentionally ungated:
  those move to `report.accepted` (their ungated finding is suppressed + excluded
  from the `'error'` gate); the rest stay in `report.ungated` (warned +
  gate-failing); `report.acceptedUnused` flags stale entries. **Anti-pattern lints
  are independent** — accepting a choice does NOT silence a `not(not(...))` /
  keyword-`regex` finding on it. Choice `id` = the rule name (bare when the rule
  holds one choice, else `rule#N`). This replaces the earlier `cold()` marker — one
  mechanism drives both warn-suppression and the CI gate.

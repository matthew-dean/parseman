...
const ALLOW = new Map([
  ...
  // INV-1. THE 36.9% DEFECT, STILL LIVE. `guardRemovedFields()` installs two
  // throwing accessors on EVERY `run()` result to explain fields removed in
  // 0.44.0. Measured at 36.9% on small parses. jess has the identical class
  // recorded at 46% of CSS parse time. The migration aid is past its
  // usefulness; deleting the function is the fix, and it is a one-line lane.
  ['INV-1:src/functional/run.ts:guardRemovedFields', 'REMOVE — measured 36.9% on small parses; migration aid for 0.44.0 removals'],

 ...
 * outside vitest was imposing a 5s ceiling on it.
 *
...
afterAll(() => {
  // `maxRetries` defaults to 0, and Node documents ENOTEMPTY as a transient
  // error for recursive removal — a loaded machine turns tidying up into a
  // suite failure. Same reason bench/size/probe.ts retries its own teardown.

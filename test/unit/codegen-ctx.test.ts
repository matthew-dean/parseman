...
describe('withCtx() codegen', () => {
  ...
  it('outer context is unchanged after withCtx', () => {
    ...
    type S = { on: boolean }
    const gateOn = gate((u) => (u as S | undefined)?.on === true)
    const p = compile(
      withCtx<S, unknown>({ on: true },
        // inside: gate passes
        sequence(gateOn, literal('a'))
      )
    )
    // Compiled version works the same as runtime
    const rt = parse(withCtx<S, unknown>({ on: true }, sequence(gateOn, literal('a'))), 'a')
    const cmp = p.parse('a')
...
describe('recursive grammar + context', () => {
  it('gate inside parser() compiles without runtime fallback', () => {

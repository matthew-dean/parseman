...
- **For `many` it recovers the entire megamorphic penalty** (91.09 → 65.61, back past the
  `identical` level of 70.17), at 725–5,830 bytes. See §5b — **this is an artifact of my
  own benchmark driver, not a property of per-site bodies.**

## 5b. The `many` wrapper anomaly — resolved, and it is a harness artifact

Probe: `bench/experiments/cliff/anomaly-probe.mjs`, trace-only (no timings — inlining
decisions are deterministic and survive a loaded box; throughput does not).

**First hypothesis, falsified cheaply.** If `many.parse` sat past the 460 B inlining
limit and the other two sat under it, the asymmetry would need no wrapper-specific
explanation. Measured body sizes (`piece-size-probe.mjs`, cold twins):
`seq.parse` **248 B**, `choice.parse` **213 B**, `many.parse` **263 B**, leaves 65 B,
every wrapper flavour 23–33 B. All three are comfortably **under** 460. Dead.

**What the trace actually shows.** In `throughput.mjs` the caller is the benchmark's own
`round()`, and the two arms originally compared differ in what *the caller* sees:

| arm | what `round` calls | piece inlined INTO the bench loop? |
|---|---|---|
| none | ONE shared FunctionLiteral, N closures | **yes** (seq 2, choice 2, many yes) |
| capture (per-site `new Function`) | N DISTINCT literals ⇒ megamorphic call site | **no** (0, all three kinds) |
| shared (one literal, 0 generated bytes) | ONE shared literal | **yes** |

The inlining structure is **identical across all three piece kinds** — `none` inlines the
piece into the bench loop, `capture` does not. Yet only `many` changes speed. So "gets
inlined into the caller" is not by itself the effect.

**The discriminator is the inner loop.** `many` is the only piece whose body loops over
its callee (8 calls/op); `seq` runs a fixed 3-iteration loop, `choice` calls once. When
`many.parse` is inlined into `benchRound` — which already has two nested loops — the
result is a triple-nested loop in a single function and TurboFan does materially worse.
The per-site wrapper "helps" only by making the caller's call site megamorphic so that
inlining is refused, restoring `many` to a clean standalone compilation.

**Consequence for D7: it gets nothing.** The win is not per-site *bytes* — it is the
piece no longer being inlined into a loop-bearing caller. The `shared` arm is the proof
in principle: same extra call level, same object shape, **zero** generated bytes. And the
benchmark's caller — a degenerate two-deep counting loop — is not representative of real
parser code, where a piece's caller is another piece.

**One timing is still owed** (deferred: the box is loaded and I will not report a number
measured under contention). The `shared` arm's throughput. The trace says `shared` is
inlined into `benchRound` exactly as `none` is, so I **predict `shared` ≈ `none` ≈ 91
ns/op**, i.e. it is specifically the per-site *literal* — not the extra call frame — that
defeats the inlining. Stated as a prediction so it can be scored, not as a result. Either
outcome leaves D7 with nothing: if `shared` is fast, a zero-byte shared trampoline buys
the same thing; if `shared` is slow, per-site bodies "help" only by defeating inlining
into a caller, which is a cost in any real caller rather than a benefit.

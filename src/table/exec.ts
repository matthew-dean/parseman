    ...
    switch (code[ip]) {
      ...
      case OP_CHOICE: {
        ...
        if (d >= 0) {
          ...
          const mark: CstRollbackMark = saveCstMark(ctx)
      ...
      case OP_REPV: {
        ...
        for (;;) {
          ...
          const cmark: CstRollbackMark = saveCstMark(ctx)
          const tmark = saveTriviaMark(ctx)
          let itemStart = cur
          if (sep >= 0 && count > 0) {
            ...
            if (sv === FAIL) {
              rollbackTrivia(ctx, tmark)
              rollbackCstCapture(ctx, cmark)
              break
            }

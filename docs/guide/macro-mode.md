...
| GraphQL | 10.3 kB | 76.6 kB | **7.4×** | 14.2 kB | 6.5× |

Larger, denser, or derived grammars — none of which the old table measured:

| Grammar | Source | Generated | **Bytes** | Gzip | Line mult. |
| --- | --- | --- | --- | --- | --- |
| lang | 4.9 kB | 41.1 kB | **8.4×** | 7.5 kB | 8.0× |
| jsonc *(variant)* | 1.1 kB | 19.1 kB | **16.9×** | — | — |
| jsonl *(variant)* | 0.8 kB | 17.1 kB | **20.7×** | — | — |
| css | 9.3 kB | 252.4 kB | **27.2×** | 39.2 kB | 23.1× |
...
Marginal cost is **≈5.2 kB per added `node()` site** across an 8× range, with an implied
fixed overhead of about zero. So `generated bytes ≈ 5.2 kB × node sites`.
...
**Each variant is emitted as a full copy.** Measured by `pnpm size:probe`, holding
the grammar constant and varying only the number of variants:
...
That is near-perfect linear duplication — nothing is shared between variants. It
is visible in the shipped artifact too: `function _r_Stylesheet(` occurs exactly
**four** times in jess's 13.1 MB `css-parser/lib/grammar.js`, as four ~3.2 MB
blocks.

A useful tell: the compression ratio climbs with variant count (5.3:1 → 8.8:1 →
12.7:1). Output that gzips unusually well is output that repeats itself.

**If you ship one grammar in several configurations, budget for a full copy of
each.** Prefer deriving variants at run time where you can, and reach for a
second compiled variant only when you actually need one.

Other measured axes, same probe:

- `trivia()` adds **1.24×**
- `hostMode: 'cst'` adds **1.08×** over `'ast'`

...
import {
  ...
  oneOrMore, transform, label, node, scanTo, sepBy, token, rules, withCtx, attempt,
} from '../../src/index.ts'
...
describe('classifyBroadArm — every cause', () => {
  ...
  it('walks through attempt (forwards first-set) to the real broad inner', () => {
    expect(anyArm(choice(attempt(broad()), literal('z'))).cause).toBe('broad-recognizer')
  })
...
describe('anti-pattern peeling', () => {
  ...
  it('detects an anti-pattern hidden behind attempt(...)', () => {
    const g = choice(attempt(sequence(not(not(literal('@'))), literal('x'))), literal('z'))
    expect(analyzeGating(g).antiPatterns.some(a => a.kind === 'double-not')).toBe(true)
  })

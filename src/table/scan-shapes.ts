/**
 * SHAPE RECOGNITION AND SOURCE EMISSION FOR SCANNABLE REGEXES.
 *
 * Restored from `archive/codegen-fastpaths:src/compiler/scannable-run.ts` and
 * re-aimed at `emit-assembly.ts`. It is a SOURCE emitter, not a runtime
 * interpreter: `emitShapeMatch` constant-folds every range and code point into
 * the text of a per-site `_pf<N>` body via `classCond`/`litCond`/`foldEq`. Built
 * instead as static runtime bodies the nine shapes would reproduce exactly the
 * `inRanges` per-character loop this file exists to remove.
 *
 * The two trivia-loop emitters (`scanBranch`, `scanBranchLabeled`) are NOT
 * restored: fusing the trivia path into piece bodies is a separate unit, and
 * carrying dead emitters here would make this file look like it already did it.
 *
...
import { CLASS_ESCAPES, SPACE_RANGES, literalCodePoints, parseClassOperand, shorthandRanges, readUnicodeEscape, parseClassRanges } from '../regex/classes.ts'
...
function trailingBacktrackClass(shape: ScanShape): CharSet | null | 'unsupported' {
  ...
  if (shape.kind === 'seq') {
    ...
    if (last.part === 'lit') {
      // A REQUIRED literal tail is fixed — no quantifier, no wiggle.
      //
      // An OPTIONAL one is the opposite, and reading it as fixed was a defect.
      // `::?(?![ \t\n\r\f])` on `":: "` matches ONE colon: the engine takes both,
      // the lookahead rejects the space, it gives the second colon back, and the
      // lookahead then sees `:` and passes. A greedy scan that resolves `::`
      // once and never reconsiders reports NO MATCH — which is what this
      // returned `null` for, so `lookaheadUnambiguous` waved it through and the
      // shape lowered wrongly. Found by the oracle on jess's css grammar the
      // first time that grammar emitted at all.
      //
      // Declined rather than modelled. The exposed class IS computable — drop
      // the literal and the char at the new end is `cps[0]` — but a chain of
      // optional parts (`a?b?`) exposes more than one level, and this analysis
      // is single-level by construction. Wrong here is a silently wrong parse;
      // declining costs one regex row.
      return last.optional ? 'unsupported' : null
    }
    if (last.part === 'run') {

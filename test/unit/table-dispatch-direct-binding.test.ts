...
describe('direct-bound dispatch topology', () => {
  it('binds fixed arms, routed flags and matcher predicates without parse-time arrays', () => {
    ...
    expect(emitted.source).toMatch(/case 0:ur=true/)
    expect(emitted.source).toMatch(/case 1:ur=false/)
    ...
    expect(routedSource).toContain('}else ur=true')
    expect(routedSource).not.toMatch(/case 0:ur=/)
    ...
    expect(plainSource).toContain('}else ur=false')
    expect(plainSource).not.toMatch(/case 0:ur=/)
  })
  ...
  it('restores routed state on throw and has behavior-bearing child/route/malformed-wire RED teeth', () => {
    ...
    const wrongRouteSource = emitted.source.replace('case 0:ur=true', 'case 0:ur=false')

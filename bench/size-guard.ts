 ...
 * place.
 *
 * Why 0.45 cannot reach it, measured rather than asserted: marginal cost is
 * ~5,170 generated bytes per `node()` site and essentially CONSTANT across a
 * 4->32 node range, so the cost is per-site, not amortised. Reaching 10x needs
 * ~1,170 B/node — a 4.5x cut — which means replacing inlined per-site preambles
 * with shared runtime helpers (the hot/cold hybrid). That is a strategy change,
 * deferred to 0.46. Every fixture still above the target gets a loud, itemised
 * warning naming the gap and the lever.
 *
 ...
 *     which still imports `compile` from `src/compiler/codegen.ts` — so they size
 *     SOURCE LOWERING, unchanged.
 *
 * That is why every `probe/*` ceiling in the baseline is byte-identical across the
 * flip while every `example/*` ceiling fell by an order of magnitude, and it is
 * why the 10x-target warning block is now populated almost entirely by probe rows.
 * The 10x number those rows are measured against is the ORIGINAL target — raw
 * generated bytes over raw source bytes — so they remain a true reading of what
 * the macro build emits. When the macro build flips too, they will move.
 *
...
async function measureProbe(): Promise<Fixture[]> {
  ...
  return units.map(u => {
    ...
    return {
      ...
      // `measure()` already fails closed on. Nothing reaches here unprintable.
      printable: true,
      reasons: [],
    }
  })
...
async function main(): Promise<void> {
  ...
  if (overTarget.length > 0) {
    ...
    if (s4 && s32) {
      ...
      console.log('    ~1,170 B/node — a 4.5x cut, which inlined preambles cannot give.')
    }
    ...
    console.log('')
    console.log('  → 0.46 plan: replace inlined per-site preambles with shared runtime helpers')
    console.log('    (hot/cold hybrid), and share across variants. Both are strategy changes,')
    console.log('    which is why 0.45 ceilings the numbers instead of pretending to hit 10x.')

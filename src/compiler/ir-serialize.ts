...
export function evalRuleMapIR(ir: string): Array<[string, Comb]> {
  ...
  const _nd = (type: string, child: Comb, src: string, opts?: unknown, staticError?: readonly string[], sigSrc?: string, buildImports?: ReadonlyArray<{ local: string; source: string; imported: string }>, rawUnused?: true, triviaUnused?: true): Comb => {
    ...
    if (triviaUnused === true) (n._def as { buildTriviaUnused?: true }).buildTriviaUnused = true
    ...
    switch (def.tag) {
      ...
      case 'node': {
        ...
        if (def.buildSrc !== undefined) {
          ...
          if (def.buildStaticError !== undefined || def.buildSigSrc !== undefined || def.buildImports !== undefined || def.buildRawUnused === true || def.buildTriviaUnused === true) {
            if (!opts) tail.push('undefined')
            tail.push(def.buildStaticError === undefined ? 'undefined' : JSON.stringify(def.buildStaticError))
          }
          if (def.buildSigSrc !== undefined || def.buildImports !== undefined || def.buildRawUnused === true || def.buildTriviaUnused === true) tail.push(def.buildSigSrc === undefined ? 'undefined' : JSON.stringify(def.buildSigSrc))
          if (def.buildImports !== undefined || def.buildRawUnused === true || def.buildTriviaUnused === true) tail.push(def.buildImports === undefined ? 'undefined' : JSON.stringify(def.buildImports))
          if (def.buildRawUnused === true || def.buildTriviaUnused === true) tail.push(def.buildRawUnused === true ? 'true' : 'undefined')
          if (def.buildTriviaUnused === true) tail.push('true')

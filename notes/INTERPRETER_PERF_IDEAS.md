...
- **Single-char `literal()` fast path** — a length-1 case-sensitive `literal()` now compares `input.charCodeAt(pos) === code` instead of calling the generic `startsWith` builtin (and the out-of-bounds `NaN` folds the length check into the compare). This is the bulk of punctuation-heavy grammars (`{ } ( ) : $ @ [ ] ! =`). A CPU self-time profile of interpreted GraphQL-large put `literal.parse` at **20.8%**; after, **5.8%**. Same-session `bench:parseman --only=... --scale=0.3 --samples=15` before→after: GraphQL large **543.9→473.8µs**, GraphQL medium **21.1→16.7µs**, CSV large **347→253µs**, lang medium **20.8→16.2µs**, JSON large **394→362µs**, TOML medium **64.7→58.7µs**. (`startsWith` vs `charCodeAt` earlier lost for the *multi-char* loop; this is strictly the single-char case.) The dead `leaf` object that `literal`/`regex`/`keywords` built before checking `cstCaptureActive()` was moved inside the guard at the same time — measured neutral (V8 already sinks the dead allocation), kept for consistency with `token.ts`.

- **Fast trivia scanner for comma/line-comment trivia** — `regexTriviaScanner()` recognized only ` \t\n\r\f` whitespace classes, so GraphQL's `(?:[ \t\n\r,]|#[^\n\r]*)*` (comma in the class, plus a `#`-to-EOL comment arm) fell through to `RegExp.exec` at every token boundary. Now: `classRunSource()` accepts any positive char class via `parseClassRanges` (so a bare `[ \t\n\r,]*` scans directly), and `altStarSource()` decomposes a `(?:arm|arm|…)*` group — splitting on top-level `|` (respecting `[…]`, bailing on nested groups), classifying each arm as a char-class run or a `C[^\n\r]*` line comment. **Arm order is not baked in**: char-class arms merge into one range list and comment markers are collected, so `(?:#…|[class])*` scans identically to `(?:[class]|#…)*`, and multiple class arms (`(?:[ \t]|[\n\r]|#…)*`) work. The disjoint common case compiles to one *fused* `charCodeAt` loop (`fusedTriviaScanner`); the one case where arm order is semantically load-bearing — a comment marker that also sits inside a class (`(?:[#]|#…)*`) — falls back to the ordered `loopScanner`. Same-session before→after (stable min-of-6 batch): GraphQL large **473.8→320µs**, GraphQL medium **16.7→12.8µs**, GraphQL small **3.0→2.2µs**; other suites unchanged. Differential-tested against the sticky `RegExp` oracle on adversarial inputs (bare/unterminated comments, comment-start after non-space, mixed commas, flipped arm order, 3 arms, and the class/marker-overlap fallback). **Net across both landings the interpreter overtook Peggy on `bench/run.ts`: GraphQL large 537→354µs vs Peggy 377µs, medium 20.5→15.0µs vs 16.1µs, small 3.44→2.18µs vs 2.40µs (was losing all three).**

Rejected alongside these (2026-07-10):
- Generalizing `regex()`'s `shortScanner` to a fixed-prefix + trailing-run shape so the `name` identifier `[_A-Za-z][_0-9A-Za-z]*` scans without `exec`. Correct (differential-tested) but measured flat — V8's irregexp already matches a simple two-class ident about as fast as the hand-rolled `inRanges` loop (GraphQL large 426.8 vs 425.4µs on/off). Left `shortScanner` at the single-class shape; do not re-add without a fresh win.
- Building the alt-star trivia scanner as a plain `loopScanner` over per-arm closures (order-independent, but one indirect call per arm per char). Clean but ~30% slower than the fused loop on GraphQL (large 322→418µs, medium 12.5→16.4µs). Kept `loopScanner` only as the overlap fallback; the disjoint case must fuse.

 ...
 *
 * A node()'s build receives (children, rawChildren, span, triviaLog, state).
 * When the build provably never declares the 4th (triviaLog) / 5th (state) formal
 ...
 * is often TypeScript — `(c: any, r: any, s: any) => …`. The arity check MUST see
...
describe('confirmedBuildArity — TypeScript annotations (the regression)', () => {
  const cases: Array<[string, number | null]> = [
    ['(c: any) => c', 1],
    ['(c: any, r: any, s: any) => x', 3], // the exact jess build shape
    ['(c: Foo, r: Bar, s: Span, tl: number[]) => x', 4],
    ['(c: any, r: any, s: any, tl: any, st: any) => x', 5],
...
describe('buildReadsTrivia / buildReadsState off buildSrc (typed)', () => {
  ...
  it('typed arity-3 → reads neither (ELIDE)', () => {
    const d = def('(c: any, r: any, s: any) => x')
    expect(buildReadsTrivia(d)).toBe(false)
    expect(buildReadsState(d)).toBe(false)
  })
  it('typed arity-4 → reads trivia only', () => {
    const d = def('(c: any, r: any, s: any, tl: number[]) => x')
...
describe('codegen elides _tl for a typed arity-3 build, keeps it for arity-4', () => {
  ...
  ;(typed3._def as Extract<ParserDef, { tag: 'node' }>).buildSrc = '(c: any, r: any, s: any) => x'
  const typed4 = node('Typed4', sequence(regex(/a/), regex(/b/)),
    (c: readonly unknown[], _r: unknown, _s: unknown, tl: readonly unknown[]) => ({ n: c.length, tl: tl.length }))
  ;(typed4._def as Extract<ParserDef, { tag: 'node' }>).buildSrc = '(c: any, r: any, s: any, tl: any) => x'
  ...
  it('typed arity-4 → allocates a per-node _tl array', () => {
    const src = compile(typed4).source

...
function captureRestoreBody(mL: string, mR: string, mTl: string, mLg: string | null, mF: string | null = null): string {
  ...
  // it intact.
  return mLg ? `${base}; if (_ctx._triviaLog && _ctx._triviaLog.length !== ${mLg}) _ctx._triviaLog.length = ${mLg}` : base
}
...
function emitFirstMatch(
  ...
  for (let i = 0; i < def.parsers.length; i++) {
    ...
    if (autoNot && autoNot.length > 0) {
      ...
      stmts.push(`${ind(ctx)}if (!${resOkV} && _ctx._fc) ${committedFailBody(ctx)}`)
    } else {
...
function emitMany(def: Extract<ParserDef, { tag: 'many' | 'oneOrMore' }>, ctx: Ctx, pos: string): ER {
  ...
  if (ctx.recovery) {
    ...
    stmts.push(`${ind(ctx)}if (!${iterOk}) { ${rollback}if (_ctx._fc) ${committedFailBody(ctx)}; break }`)
...
function emitOptional(def: Extract<ParserDef, { tag: 'optional' }>, ctx: Ctx, pos: string): ER {
  ...
  const stmts = [
    ...lblStmts,
    `${ind0}if (!${okVar} && _ctx._fc) ${committedFailBody(ctx)}`,
...
function emitAttempt(p: Combinator<unknown>, def: Extract<ParserDef, { tag: 'attempt' }>, ctx: Ctx, pos: string): ER {
  ...
  const rollback = `if (_ctx._cstLeaves && _ctx._cstLeaves.length !== ${leaves}) _ctx._cstLeaves.length = ${leaves}; if (_ctx._cstRawChildren && _ctx._cstRawChildren.length !== ${raw}) _ctx._cstRawChildren.length = ${raw}; if (_ctx._cstTriviaLog && _ctx._cstTriviaLog.length !== ${trivia}) _ctx._cstTriviaLog.length = ${trivia}; if (_ctx._triviaLog && _ctx._triviaLog.length !== ${log}) _ctx._triviaLog.length = ${log}; if (_ctx._fields && _ctx._fields.length !== ${fields}) _ctx._fields.length = ${fields}; if (_ctx._errors && _ctx._errors.length !== ${errors}) _ctx._errors.length = ${errors}`
  ...
  return {
    stmts: [
      ...
      ...inner.stmts,
      ...emitIfFail(ctx, `!${inner.okVar}`, `{ ${rollback};${traceRollback} if (_ctx._fc) ${committedFailBody(ctx)}; _ctx._fe = ${pos}; ${propagateFailBody(ctx)} }`),
    ],
...
function emitSepBy(_p: Combinator<unknown>, def: Extract<ParserDef, { tag: 'sepBy' }>, ctx: Ctx, pos: string): ER {
  ...
  const failItem = (nextOkVar: string, itemPosVar: string, breakStmt: string): string[] => {
    ...
    if (!rec) return [`${ind(ctx)}if (!${nextOkVar}) { if (_ctx._fc) ${committedFailBody(ctx)}; ${breakStmt} }`]
    const rr = v(ctx, '_rr')
    return [
      `${ind(ctx)}if (!${nextOkVar}) {`,
      `${ind(ctx)}  if (_ctx._fc) ${committedFailBody(ctx)}`,
  ...
  if (rec) {
    ...
    stmts.push(
      ...
      `${ind(ctx)}else if (_ctx._fc) ${committedFailBody(ctx)}`,
    ...
    stmts.push(`${ind(ctx)}if (!${firstOk} && _ctx._fc) ${committedFailBody(ctx)}`)
  ...
  if (ctx.activeTrivia) {
    if (ctx.capturing) {
      ...
      stmts.push(...sepStmts, `${ind(ctx)}if (!${sepOk}) { ${rollbackToSep}if (_ctx._fc) ${committedFailBody(ctx)}; break }`)
      ...
      sepAtPos = spV
      const { stmts: sepStmts, okVar: sepOk, endVar: sepEnd } = emitFallible(def.separator, ctx, sepAtPos, true)
      stmts.push(...sepStmts, `${ind(ctx)}if (!${sepOk}) { if (_ctx._fc) ${committedFailBody(ctx)}; break }`)
...
function emitNot(def: Extract<ParserDef, { tag: 'not' }>, ctx: Ctx, pos: string): ER {
  ...
  return {
    stmts: [
      ...
      ...stmts,
      ...(sinksLive ? [
        `${ind(ctx)}if (_ctx._cstLeaves && _ctx._cstLeaves.length !== ${leaves}) _ctx._cstLeaves.length = ${leaves}; if (_ctx._cstRawChildren && _ctx._cstRawChildren.length !== ${raw}) _ctx._cstRawChildren.length = ${raw}; if (_ctx._cstTriviaLog && _ctx._cstTriviaLog.length !== ${tl}) _ctx._cstTriviaLog.length = ${tl}; if (_ctx._triviaLog && _ctx._triviaLog.length !== ${log}) _ctx._triviaLog.length = ${log}; if (_ctx._fields && _ctx._fields.length !== ${fields}) _ctx._fields.length = ${fields}; if (_ctx._errors && _ctx._errors.length !== ${errors}) _ctx._errors.length = ${errors}`,
      ] : []),

...
A bare `regex(/color/)` and the hand-rolled `sequence(literal('color'), not(regex(/\w/)))`
both work, but `word('color', '-\\w')` is shorter, has an exact first-set, and lowers to
the same `charCodeAt` scan. Reach for `regex` only when the token is a real pattern.

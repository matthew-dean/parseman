 ...
 *
 * The table lowering's headline advantage over codegen is a far faster cold
 * import, so a fold that saved bytes by making load slower would be trading the
 * thing the lowering is for. It does the opposite, and this measures by how
...
async function one(dialect: Dialect): Promise<void> {
  ...
  for (let i = 0; i < WARMUP + REPS; i++) {
    ...
    for (const v of VARIANTS) tableRules(a[v]!)
  ...
  const mn = median(neu)
  console.log(
    `  ${dialect.padEnd(6)} four tables ${mo.toFixed(2)} ms   folded ${mn.toFixed(2)} ms   `

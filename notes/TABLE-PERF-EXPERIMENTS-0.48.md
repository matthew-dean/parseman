...
| T14 | P0 | Let direct terminal negative lookahead consume the shared scalar recognizer/pending range instead of entering and rolling back a terminal piece. This is a token-compatible consumer, not a competing scanner. | Interpreter/reference/closure/emitted/precompiled identity; proven-RED assertion directions; probe/expected/commit/sink/coverage/tracked semantics; unchanged program words; canonical Less A/A+A/B and package cost. | `json_fixed_regex` | CHECKPOINT READY / CANONICAL TIMING PENDING — rebased checkpoint `801d3808` has all strict differentials and focused semantics green with zero opcode/layout changes. Corrected quick screens measured about 11% faster on both standard Less rows against tight controls; full timing waits for the machine's hard load ceiling. |

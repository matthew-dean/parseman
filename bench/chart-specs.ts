...
// ── Small rows are release evidence again in 0.48 ───────────────────────────
//
// These 27–54 byte inputs expose fixed per-call overhead that medium and large
// rows amortise away. They were temporarily omitted while the comparison harness
// could not distinguish their tightest margin from its own noise. The margin gate
// now measures and enforces an in-run A/A control, so an unresolved small row
// yields INDETERMINATE instead of a false rank claim. Keep all four small rows
// together: selecting only the wins would be benchmark cherry-picking.

...
(`src/combinators/choice.ts`), and the **table lowering** used by `compile()` and the
macro build resolves the same dispatch when it assembles the table
(`src/table/assemble.ts`). The diagnostics below report the gating

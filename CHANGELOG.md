...
- **Capture restores are shared through hoisted helpers instead of inlined at every
  ...
  path of a speculative branch, so it is cold BY CONSTRUCTION — no corpus and no
  execution-count estimate is needed to classify it, which is what makes this a
  deterministic hot/cold split rather than a guess. The success path is untouched: at
  the two sequence-boundary sites the `if (endAfter > scanEnd)` compare stays inline and
  only the non-advancing `else` body is shared. The guarded `if (sink && sink.length !==
  mark)` form is preserved verbatim inside the helpers — a bare length store cost +32%
  on jess's Less grammar in 0.34.0 — and `rollback-store-guard-parity` now checks the
  helper bodies too, rather than passing vacuously once the inline form disappears.
  Generated size falls **3.0%** across the seven example fixtures (example/css 26.7x →
  25.3x, its single largest improvement); `bench/size-baseline.json` is rebaselined
  accordingly.
- **The restore text is emitted from ONE place.** It had been hand-rolled at seven
  independent sites in `codegen.ts` (`captureRestoreBody` plus six template literals in
  the choice, `many`, `sepBy`, `not` and dispatch emitters), which is why the first cut
  of the change above measured 1.3% instead of the predicted 10%: only two of the seven
  went through the shared function. Consolidating them also normalises the separators
  those copies had drifted apart on, worth 0.43% on its own with no behavioural change
  and no added call.
- **Not reached: the 10x size target.** 0.45 deferred it to 0.46 on the reading that
  ~5,170 generated bytes per `node()` site was a per-node preamble that a hot/cold split
  could mostly remove. Decomposed against the probe's own instrument
  (`bench/size/decompose.ts`), that number is bytes per *rule* — the probe's generator
  adds a `node()` wrapper, a three-term `sequence`, and one more `choice` arm per
  increment. The node preamble is **11%** of it (545 of 4,937 B); the body and the arm
  are the other 89%. A semantic census of the real fixtures
  (`bench/size/category.ts`) shows no dominant mechanism at all: the largest is control
  flow at 22%, and the top 30 normalised line shapes together are only 37% of generated
  bytes. There is no 4.4x lever here, and 10x is not reachable by sharing cold paths.

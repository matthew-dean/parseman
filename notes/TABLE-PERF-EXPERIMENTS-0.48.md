...
| T04a | Retired prototype; no commit | A memoized table-IR effect proof removed a non-exclusive choice's mark/rollback when no arm could publish state before failure. Its first model went RED on the real emitted-vs-closure Less corpus: `at-rules.less` and `css-3.less` matched on value/span/expected/tree but leaked balanced-scan errors through a failed `node()`, proving that node-private CST/fields do not make `_errors` private. Propagating opaque error effects restored exact identity for all 16 CSS/Less/SCSS/Jess × AST/AST-lines/CST/CST-lines corpus lanes, including 2,408 SCSS files. Emitted source shrank CSS 1,019,730→993,143 B (-2.61%), Less 1,998,680→1,945,750 B (-2.65%), SCSS 1,369,824→1,331,837 B (-2.77%), and Jess 1,442,998→1,421,180 B (-1.51%). Exact-parent `5b8efa8` production timing was flat: Less stylesheet -1.9%…+1.0%, Less mixins -0.5%…+0.8%, CSS +0.2%…+2.6%, GraphQL +1.0%…+1.8%, and noisy JSON -1.3%…+11.5% with only 1/5 breaches; load was 7.98→5.42. The intended rollback-density rows were also flat: none +0.0%…+2.1%, sparse -0.5%…+0.8%, medium +0.0%…+4.3% (2/5 breaches), dense -0.3%…+1.7%; load 5.13→4.09. | REJECTED. The only deterministic benefit was smaller transient generated source; parse time did not improve, the medium-density case leaned slower, and retaining the proof would add a second subtle sink-effect lattice plus require a closure-engine twin without recovering a release shelf. The prototype was removed cleanly. |

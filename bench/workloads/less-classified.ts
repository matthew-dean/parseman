/**
 * `less.ts` with LABELLED trivia, for the ctx-shape A/B only.
 *
 * Identical grammar, identical corpus handling; `classifiedTrivia()` replaces
 * `trivia()` so `run({ rootTrivia: { select: [...] } })` is reachable. The broad
 * gate's workload set is untouched — this file adds no row to it.
 */
/**
...
// LABELLED trivia — the only difference from `less.ts`. `run({ rootTrivia })`
// refuses without labels, and selected root trivia is the configuration the
// ctx-shape A/B has to cover. Same three arms, same order.
const rw = classifiedTrivia({ whitespace: ws, lineComment, blockComment })
...
/** Exported so the ctx-shape A/B can pass the same labelled trivia to `run()`. */
export { rw }

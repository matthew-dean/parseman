...
describe('small-choice token predecision', () => {
  ...
  it('rejects a multi-character literal arm in a larger ordered choice', () => {
    const grammar = choice(
      attempt(transform(sequence(literal('@@'), literal('!')), () => 'indirect')),
      transform(sequence(literal('@'), literal('?')), () => 'variable'),
      transform(sequence(literal('$'), literal(':')), () => 'dollar'),
      transform(sequence(literal('%'), literal('.')), () => 'percent'),
    )

    const source = expectIdentity(grammar, ['@@!', '@?', '$:', '%.', '@@?', '@:', 'x'])
    expect(source).toMatch(/&&_rec\d+\(input,pos\)>=0/)
  })
